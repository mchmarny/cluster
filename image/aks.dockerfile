# AKS v2 Actuator Image
# Self-contained Terraform deployment with Go CLI entrypoint
# Supports: linux/amd64, linux/arm64
#
# Build:
#   ./tools/mirror aks           # optional: pre-cache TF providers for offline
#   docker build -f image/aks.dockerfile -t cluster:dev .
#
# Run:
#   docker run --rm cluster:dev --version
#   docker run --rm -e CONFIG_CONTENT \
#     cluster:dev apply
...
# kubelogin (AAD auth plugin for kubectl)
RUN set -eux; \
...
# Azure CLI (Python package installed into a relocatable venv)
RUN set -eux; \
    python3 -m venv /opt/venv && \
    /opt/venv/bin/pip install --no-cache-dir --upgrade pip && \
    /opt/venv/bin/pip install --no-cache-dir "azure-cli==${AZURECLI_VERSION}"
...
COPY --from=tools /usr/local/bin/kubelogin /usr/local/bin/kubelogin
...
# AKS Terraform source (always available)
COPY --chown=builder:builder provider/aks/terraform /builder/terraform
...
ARG KUBECTL_VERSION=1.36.1
ARG AZURECLI_VERSION=2.87.0
LABEL org.opencontainers.image.title="AKS v2 Actuator" \
      org.opencontainers.image.description="Self-contained Terraform deployment for Azure AKS" \
      org.opencontainers.image.version="${VERSION}" \
      terraform.version="${TERRAFORM_VERSION}" \
      kubectl.version="${KUBECTL_VERSION}" \
      azurecli.version="${AZURECLI_VERSION}"
