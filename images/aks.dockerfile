...
# Install Azure CLI (architecture-independent via pip)
RUN pip3 install --break-system-packages azure-cli==${AZCLI_VERSION} \
    && az version
...
LABEL org.opencontainers.image.title="AKS Actuator"
LABEL org.opencontainers.image.description="Self-contained Terraform deployment for Azure AKS"
LABEL terraform.version="${TERRAFORM_VERSION}"
LABEL kubectl.version="${KUBECTL_VERSION}"
LABEL yq.version="${YQ_VERSION}"
LABEL azcli.version="${AZCLI_VERSION}"
